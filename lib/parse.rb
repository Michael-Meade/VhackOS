...
# Makes sure that the Gems are installed
begin
...
module Parse
  class << self
    public
    def Comma(*args)
        # Used to edit numbers so they have commas
        begin
    ...
    def WithdrawType(*args)
        # Check whenever if able to withdraw
        args = args.shift
        case args 
        when "0"
    ...
    def CheckBrute(*args)
        # Gets the status of the Brute task
        args = args.shift
        case args
        when "0"
    ...
    def UnixConvert(*args)
        # Convert Unix to regual time
        # Game server uses Unix
        args = args.shift
        return Time.at(args.to_i).strftime("%b %e %I:%M %p").gsub('[', "").gsub(']', "").gsub('"', "")
    end
    def check_string(*args)
        # Checks to make sure the inputed is a number
    ...
    def ParseRemoteBanking(username, target)
        # Makes the tarets remote banking easier to read
        rows = TTY::Table.new
    ...
    def ParseBanking(username)
        ...
        length = bank["transactions"].length
            # Loops thru the transaction length
            for i in 0..length
                # Used to safely end the loop
                if i.to_i == length.to_i
                    break
                end
                # Puts all the banking info into an array
                rows2 << [
    ...
    def ParseNetwork(username)
        # Creates the TTY table
        # Also defiens the tables Headers
        row = TTY::Table.new header: ['Ip', 'fw', 'Level', 'Open']
        # Get the authorized user network info
        network = VhackXt.GetNetwork(username)
        # Get how many IPs
        length  = network["ips"].length
        for i in 0..length.to_i
            if i.to_i == length.to_i
                # Used to safetly end the Loop
                break
            end
        # Creates a Array that the TTY gem used to create a table
        row << [
                    network["ips"][i]["ip"],
    ...
    def ParseMoneyExploit(username, target, amount)
        # Used to parse the money exploit... They patched the stupid exploit
    ...
    def ParseBrute(username)
        # Used to create a table of all the Brute tasks
        row = TTY::Table.new header: ['IP', 'Username', 'Start', 'End', 'Now', 'Status']
        brute  = VhackXt.BruteList(username)
        if brute["brutes"].nil?
            # Alerts end user if there are no brute task running
    ...
    def ParseSDK(username)
        # Creates a nice looking table of the SDK
        rows = TTY::Table.new 
    ...
    def SeverPieces(username)
        # Creates a nice table of the server pieces
    ...
    def ParseLogIn()
        # Used to create a option table to let the user decide if they want
        # to login in by username and pass or want to login by file
        rows = TTY::Table.new 
        rows << ['1',    'Login Normal']
        rows << ['2',    'File Login']
        puts rows.render(:unicode, alignment: [:center])
    ...
    def ParseCreateLog()
        # Creates a option table that helps keep stuff look nice
        row = TTY::Table.new
    ...
    def ParseSpam(username)
        # Parse spam income
        # Adds commas to Numbers that need it
        spam = VhackXt.GetSpam(username)
        spam_info = [
