...
module VhackXt
  class << self
    ...
    def Main(php, username, userAndPass)
        user = userAndPass[0]
        pass = userAndPass[1]
        url = "https://api.vhack.cc/mobile/36/#{php}?user=#{user}&pass=#{pass}"
    ...
    def login(username, password)
        ...
        user = userAndPass[0]
        pass = userAndPass[1]
        url = "https://api.vhack.cc/mobile/36/login.php?user=#{user}&pass=#{pass}"
        ...
        if json.nil?
            ...
            user = getUserInfo[0]
            pass = getUserInfo[1]
            url = "https://api.vhack.cc/mobile/36/update.php?user=#{user}&pass=#{pass}"
            response = open(url, 'User-Agent' => 'Mozilla/5.0 (Linux; Android 5.1.1; SM-G928X Build/LMY47X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.83 Mobile Safari/537.36').read
    ...
    def NewAccountBuy(username)
            ...
            #user_data = VhackXt.Main("store.php", username, userAndPass)
            ["0", "3", "6", "4"].each do |i|
                name = self.GetAppType(i)
                puts "Bought #{name}."
                VhackXt.DownloadApp(username, i)
            end
            for i in 0..10
    ...
    def Uploadbg(username, img)
        login_info = ReadJson(username)
        login_info["uid"] = login_info["uid"]
        login_info["img"] = File.open(img).read
        puts login_info
        userAndPass = GenerateUserAndPass(login_info)
        user_data = VhackXt.Main("uploadbg.php", username, userAndPass)
        puts user_data
    end
    ...
    def GetLog(username)
        # Get authorized User's log
        login_info = ReadJson(username)
        getUserInfo = GenerateUserAndPass(login_info)
        user = getUserInfo[0]
        pass = getUserInfo[1]
        url = "https://api.vhack.cc/mobile/36/log.php?user=#{user}&pass=#{pass}"
        response = open(url, 'User-Agent' => 'Mozilla/5.0 (Linux; Android 5.1.1; SM-G928X Build/LMY47X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.83 Mobile Safari/537.36').read
        user_data = JSON.parse(response)
        return user_data
    end
    ...
    def AddCustomLog(fileName, text)
        # Create a text file to save a log
    ...
    def WriteToRemoteLog2(username, log, target)
        login_info = ReadJson(username)
        login_info["target"] = target
        login_info["log"] = log
        login_info["action"] = 100
    ...
    def AutoExploit(username)
        # Exploit targets in the game; spends all your exploit credits
    ...
    def ExploitedList(username)
        user_data = GetNetwork(username)
        return user_data
    end
    def SendUsername(username, ip, uName)
        # Used this to create a "Database" of users... Everytime its called it will send the information to my vps
        # Creating a search database
        # Havnt been used since version 3
    ...
    def SpamExploit(username, target)
        succesfully = 0
        notsuccess  = 0
        login_info = ReadJson(username)
        login_info["target"] = target
        ...
        begin
            ...
            #puts "succesfully: #{succesfully}"
            #puts "NOT: #{notsuccess}"
            return succesfully, notsuccess
        rescue => e
            puts "Exception Message: #{ e.message }".red
            puts "Retrying...... please wait 20 seconds.".red
            sleep(20)
